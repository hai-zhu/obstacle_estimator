...
bag_100Hz = rosbag('wifi_100Hz.bag');
...
plot(pos_100Hz(:,1), pos_100Hz(:, 5), '-b');
plot(pos_reactive(:,1), pos_reactive(:, 5), '-r');
legend('raw', '100Hz');
...
xlabel('time stamp [s]');
ylabel('velocity [m]');
axis([0 max(duration_100Hz, duration_reactive) -10.0 10.0]);
plot(vel_100Hz(:,1), vel_100Hz(:, 5), '-b');
